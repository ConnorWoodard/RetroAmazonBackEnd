...
debugUser.color = '63';
import { addUser, getAllUsers, loginUser, newId, getUserById, updateUser, saveEdit, findRoleByName} from '../../database.js';
...
router.post('/login', validBody(loginUserSchema), async (req,res) => {
    ...
    if(resultUser && await bcrypt.compare(user.password, resultUser.password)){
        ...
        issueAuthCookie(res, authToken);
        res.status(200).json({message: `Welcome ${resultUser.fullName}.`, email:resultUser.email, authToken:authToken});
    }else{
...
router.put('/update/:id', isLoggedIn(), validId('id'), validBody(updateUserSchema), async (req,res) => {
    ...
    debugUser(`The user is ${JSON.stringify(user)}`);
